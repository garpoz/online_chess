...
var Client = (function(window) {
    function fullTime(){
        ...
        if(winer===playerColor){
            showGameOverMessage('forfeit-win');
            xhr.open('GET',`/?winuser=${lStorage}&reward=${gameID.slice(7)}baba`);
            xhr.setRequestHeader('content-Type','application/json');
            xhr.send();
        }else{
            showGameOverMessage('forfeit-lose');
            xhr.open('GET',`/?losuser=${lStorage}&reward=${gameID.slice(7)}baba`);
            xhr.setRequestHeader('content-Type','application/json');
            xhr.send();
        }
    }
        ...
        var winer=null;
    var lStorage=localStorage.getItem("gMail");
    var stop=null;
    var istop=null;
  var socket      = null;
  var gameState   = null;
  ...
  var forfeitPrompt       = null;
            ...
            var blackStr=blackStr||true;
            var whiteStr=whiteStr||true;
var sec=1;
var min=5;
var hor=0;
var ssec="";
    ...
    for (var i=0; i<gameState.players.length; i++) {
      ...
      if (gameState.activePlayer && gameState.activePlayer.color === gameState.players[i].color) {
        ...
        if(gameState.activePlayer.color==="white"){
            wColor=$("#timer-white");
            if(istop){istop=false;stop=true;}else{istop=true;stop=false;}
            if(whiteStr===true){
                whiteStr=false;
                stop=false;
var enterwhite=setInterval(displaytimer, 1000);
    ...
    val=`${shor}:${smin}:${ssec}`;
              wColor.text(val);
                    if(min<=1){
                        $(wColor).css('color', 'red');
                        if(sec===0 && min===0){
                            fullTime();
                        }
                    }else{
                        $(wColor).css('color', 'white');
        ...
        if(gameState.activePlayer.color==="black"){
            iwColor=$("#timer-black");
            if(stop){istop=true;stop=false;}else{istop=false;stop=true;}
            if(blackStr===true){

                blackStr=false;
                istop=false;
var enterblack=setInterval(idisplaytimer, 1000);
function idisplaytimer(){
                    if(istop===true){return 0;}
    isec--;
    if (isec===0 && imin!==0){
        isec=59;
        imin--;
    ...
    if(isec<10){
        issec="0"+isec.toString();
    }else{
        issec=isec.toString();
    }
    if(imin<10){
        ismin="0"+imin.toString();
    }else{
        ismin=imin.toString();
    }
    if(ihor<10){
        ishor="0"+ihor.toString();
    }else{
        ishor=ihor.toString();
    }
    ival=`${ishor}:${ismin}:${issec}`;
              iwColor.text(ival);
                    if(imin<=1){
                        $(iwColor).css('color', 'red');
                        if(imin===0 && isec===0){
                            fullTime();
                        }
                    }else{
                        $(iwColor).css('color', 'black');
  ...
  var showGameOverMessage = function(type) {
    ...
    switch (type) {
      case 'checkmate-win'  : header.addClass('alert-success').text('You Win⭐ '+(gameID.slice(7)-((gameID.slice(7)*10)/100)).toString()); break;
      case 'checkmate-lose' : header.addClass('alert-danger').text('You Lose '+gameID.slice(7)); break;
      case 'forfeit-win'    : header.addClass('alert-success').text('You Win⭐ '+(gameID.slice(7)-((gameID.slice(7)*10)/100)).toString()); break;
      case 'forfeit-lose'   : header.addClass('alert-danger').text('You Lose '+gameID.slice(7)); break;
